...
void takeInput()
{
	...
	// cin>>goalStr;
	startStr = "on(B,A),ont(C),ont(A),ont(D),cl(B),cl(C),cl(D),ae";
	goalStr = "on(C,A),on(B,D),ont(A),ont(D),cl(C),cl(B),ae";
...
void modifyState(string str)
{
	...
	{
		tcur = &currst;
		while(tcur!=NULL && del->state.compare(tcur->state) == 0)
		{
			tcur = tcur->next;
			currst = *tcur;
		}
